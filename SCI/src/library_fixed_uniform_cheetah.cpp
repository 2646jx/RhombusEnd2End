...
void Conv2DWrapper(signedIntType N, signedIntType H, signedIntType W,
  ...
  printf(
      ...
      Convctr++, N, meta.ishape.height(), meta.ishape.width(),
...
void Conv2DWrapper_1m1Filter(uint32_t H, uint32_t W, uint32_t CI,
                                    uint32_t CO, uint32_t stride,
                                    uint64_t *inputArr, uint64_t *filterArr, uint64_t *outArr)
{
    ...
    printf(
      "HomConv #%d called (Rhombus) N=%ld, H=%ld, W=%ld, CI=%ld, FH=%ld, FW=%ld, "
      "CO=%ld, S=%ld, Padding %s (%d %d %d %d)\n",
      Convctr++, 1, H, W, CI, 1, 1, CO, stride, "VALID", 0,
      0, 0, 0);

    uint64_t time_, sent_;
    if (party == 1){
        time_start = std::chrono::high_resolution_clock::now();
        uint64_t before_sent = rhombus_linear->IO()->counter;

        rhombus_linear->Conv2D_1m1Filter(H, W, CI, CO, stride, inputArr, filterArr, outArr);

        time_end = std::chrono::high_resolution_clock::now();
        time_diff = std::chrono::duration_cast<std::chrono::microseconds>(time_end - time_start);

        time_ = time_diff.count();
        ConvTimeInMilliSec_Rhombus += uint64_t(time_diff.count() / 1000.);
        sent_ = (rhombus_linear->IO()->counter - before_sent);
        ConvCommSent_Rhombus += (rhombus_linear->IO()->counter - before_sent);
    }else{
        time_start = std::chrono::high_resolution_clock::now();
        uint64_t before_sent = rhombus_linear->IO()->counter;
...
#ifdef LOG_LAYERWISE
  std::cout << "Time in sec for current conv = [" << (time_ / 1000. / 1000.0)
            << "] sent [" << (sent_ / 1024. / 1024.) << "] MB"
            << std::endl;
#endif
}
